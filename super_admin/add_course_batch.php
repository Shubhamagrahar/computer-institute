...
if(isset($_POST['Create'])){
  ...
  if(!$batch_name==""){
     $check=mysqli_num_rows(mysqli_query($con,"select * from batch_details where batch_name='$batch_name'"));
     if(!$check>0){
         $insert=mysqli_query($con,"insert into `batch_details`(`batch_name`) values('$batch_name')"); 
        if($insert){
