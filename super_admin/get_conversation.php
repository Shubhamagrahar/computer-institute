...
if (isset($_GET['convers_id']) && isset($_GET['receiver_id']) && isset($_GET['userid'])) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $user_details = mysqli_fetch_array(mysqli_query($con, "select name, mobile from user where id='$userid'"));
        ...
        if ($row['role'] == 1) {
            ...
            echo "<div style='display: flex; justify-content: flex-start;'> 
                    <p style='background-color: #6c757d; color: white; padding: 10px; border-radius: 10px;
                                max-width: 70%; margin: 5px 0; display: inline-block;'><strong>" . $user_name . ":</strong> ";
        }
...
if (isset($_GET['query_id'])) {
    ...
    $update = mysqli_query($con, "UPDATE query SET status='2' WHERE convers_id='$convers_id'");
