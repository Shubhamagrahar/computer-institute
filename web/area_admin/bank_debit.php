     ...
     $check=mysqli_query($con,"select * from user where branch_id='$_SESSION[userid]' and mobile='$mobile'");
      if(mysqli_num_rows($check)==1){
       ...
       if($update_wallet){
           ...
           $teransactionIntsert=mysqli_query($con,"insert into `transaction`(`userid`, `des`, `debit`, `type`, `date`, `c_date`, `op_bal`, `cl_bal`, `by_userid`) values('$result[id]', '$desciption', '$amt', '2', '$t_date', '$c_date', '$op_bal', '$cl_bal', '$_SESSION[userid]')");
           if($teransactionIntsert){
               echo '<script>alert("Amount debited successfully done.");window.location.assign("bank_debit");</script>'; 
           }else{
         ...
         echo '<script>alert("Server Error 101.");window.location.assign("bank_debit");</script>';   
          ...
          echo '<script>alert("Mobile Number Not Registered.");window.location.assign("bank_debit");</script>'; 
      }
