...
if(isset($_GET['search'])){
    error_log($current_branch_id);
    $reg_no=VerifyData($_GET['reg_no']);
    if(!$reg_no==""){
        $check=mysqli_query($con,"select * from user where branch_id='$current_branch_id' and reg_no='$reg_no'");
        if(mysqli_num_rows($check)==1){
            $user_details=mysqli_fetch_array($check);
            $search_data=2;
            
        }else{
          echo '<script>alert("Registration number not registered or not in your franchise.");window.location.assign("student_search");</script>';   
        }
    }else{
                ...
                <h3 class="card-title">Student Details Search By Registration Number</h3>
                 ...
                 <div class="card-body">
                  <div class="row">
                      ...
                      <input type="reg_no"  name="reg_no" required value="" class="form-control" placeholder="Enter Registered Number.">
 ...
 <script>
      ...
      function fee_paid(val){
          ...
          if(pay_fee>0){
                ...
                btn.prop("disabled", true).html('<i class="fa fa-spinner fa-spin"></i> Processing...');
                        ...
                        success: function(data){
                          ...
                          if(data==1){
                             ...
                             Swal.fire({
                                 title: "Success",
                                 text: "Fee has been collected sucessfully!.",
                                 icon: "success"
                             }).then(() =>{
                                 window.location.assign("student_search");
                             });
              ...
              Swal.fire("Warning","Please enter pay fee amount.", "warning");
          }
      }
