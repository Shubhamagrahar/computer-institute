        ...
        if(!$sql_check>0){
           $extension12 = explode(".", $photo);
            ...
            if($upload_photo){
               ...
               if($insert){
                   $insert_id=$count_id;
                    $new_insert_id = mysqli_insert_id($con);
                if($insert_id>0){
                    $insert_wallet=mysqli_query($con,"insert into `wallet`(`userid`) values('$new_insert_id')");
                     if($insert_wallet){
                         $create_staff_details=mysqli_query($con,"insert into `staff_details`(`userid`, `qualification`, `designation`, `doj`, `monthly_salary`) values('$new_insert_id', '$qualification', '$designation', '$doj', '$monthly_salary')");
