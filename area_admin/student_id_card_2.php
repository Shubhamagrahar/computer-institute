    ...
    if(!$data_id==""){
      ...
      if(mysqli_num_rows($sql)==1){
        ...
        //  $dob=date_format($dob,"d-M-Y");
         ...
         $branch_details=mysqli_fetch_array(mysqli_query($con,"select * from user where id='$result[branch_id]'"));
...
<link href="https://fonts.googleapis.com/css2?family=Belanosima&family=Poppins:wght@500&family=Slabo+27px&family=Staatliches&display=swap" rel="stylesheet">
		<style>
		body{
	...
	<div style="position: relative; width:715px; height:1021px;">
					<div style="position:absolute;margin: 7px 0 0 59px;font-weight: bold; width: 166px; font-size: 18px; color: #ffffff;font-family: 'Staatliches', cursive;line-height: 18px;word-spacing: 0px;letter-spacing: 1px;">
			<?php echo $branch_details['name'];?>
						</div>
				
			
					
					<div style="position:absolute; margin: 183px 0 0 3px; font-weight: bold; color: #ffffff; font-size: 14px;min-width: 157px; max-width: 157px; width: 157px;">
		<?php echo $name; ?>
						</div>
				...
				Date of issue :	<?php echo $course_book_date; ?>
					...
					<div>
					    <img class="thing" src="<?php echo $web_link.$branch_details['photo'];?>" style="position: absolute;background:white; margin: 4px 0 0 4px;width: 46px;height: 46px;border-radius: 65px;">		
					</div>
					<div>
				...
				<img src="https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=<?php echo $name; ?>/<?php echo $id; ?>&size=110x110" style="position: absolute; margin-top: -8px;  margin-left: -6px; width: 67px;">	
					</div>
					
										
					<img  src="image/../super_admin/id_card_2.jpg" style="width:230px; height:345px; border:1px solid black; border-radius: 7px;" />
