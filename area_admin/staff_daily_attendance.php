    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            if(!$_SESSION['search_date']==""){
                ...
                $sql=mysqli_query($con,"select * from user where branch_id='$branch_id' and type='2' and status='1'");
                while($row=mysqli_fetch_array($sql)){
                   $sql_check=mysqli_num_rows(mysqli_query($con,"select * from attendance_staff where branch_id='$branch_id' and userid='$row[id]' and date='$_SESSION[search_date]'"));
                    if(!$sql_check>0){ 
                     $sql_insert=mysqli_query($con,"insert into `attendance_staff`(`branch_id`, `userid`, `date`, `create_by`) values('$branch_id', '$row[id]', '$_SESSION[search_date]', '$_SESSION[userid]')");
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <label>Select Date:</label>
                             <input type="date" value="" name="search_date" class="form-control" onchange="window.location.assign('staff_daily_attendance?search_date='+this.value)">
                        </div>
