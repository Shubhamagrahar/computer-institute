...
if (!empty($_GET['reg_no']) && !empty($_GET['dob'])) {
    $reg_no = $_GET['reg_no'];
    $dob = $_GET['dob'];
    $user_query = mysqli_query($con, "SELECT * FROM user WHERE reg_no='$reg_no' and dob = '$dob' and status='1'");
  
    
    $user = mysqli_fetch_array($user_query);
     $course = mysqli_fetch_array(mysqli_query($con, "select * from course_book where userid='$user[id]' and status='RUN'"));
   $course_name = mysqli_fetch_array(mysqli_query($con, "select name from course_details where id='$course[course_id]'"))['name'];
   $start_date = date('d-M Y', strtotime($course['start_date']));
    if (mysqli_num_rows($user_query) == 0) {
        echo "<script>
            alert('Please Enter Valid Registration Number and Date of Birth');
        ...
        <div class="col-md-9 mx-auto">
            ...
            <div class="student-info-container">
                ...
                <div class="student-details-table">
                    <table>
                        <tr><th>Student Name:</th><td><?php echo $user['name']; ?></td></tr>
                        <tr><th>Father's Name:</th><td>Mr. <?php echo $user['father_name']; ?></td></tr>
                        <tr><th>Date of Birth:</th><td><?php echo date('d-M Y', strtotime($user['dob'])); ?></td></tr>
                        <tr><th>Gender:</th><td><?php echo $user['gender']; ?></td></tr>
                        <tr><th>Address:</th><td><?php echo $user['full_add']; ?></td></tr>
                        <tr><th>Course Name:</th><td><?php echo $course_name; ?></td></tr>
                        <tr><th>Course Start Date:</th><td><?php echo $start_date; ?></td></tr>
                        <tr><th>Institute's Name:</th><td><?php echo $brand_name; ?></td></tr>
                        <tr><th>Institute's Address:</th><td><?php echo $brand_add; ?></td></tr>
                    </table>
                </div>
            </div>
        </div>
