...
$session = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM login_session WHERE userid='$login_details[id]'"));
...
if(isset($_GET['ids'])){
    ...
    if(!$course_book_id==""){
       ...
       if(mysqli_num_rows($sql)==1){
          ...
          $user_details=mysqli_fetch_array(mysqli_query($con,"select * from user where id='$book_course_details[userid]' and branch_id='$current_branch_id'"));
       }else{
     ...
     // if(!$photo==""){
          if(!$fee_collect_type==""){
              ...
              if($fee_collect_type=="NO"){
                  ...
                  $course_final_fee=VerifyData($_POST['course_final_fee']);
                 if($course_final_fee == ""){
                    echo '<script>alert("Please verify final fee before proceeding with part payment."); window.location.assign("enroll_final");</script>';
                    exit();
                }
              }
        ...
        if($upload_photo){
            ...
            $unlink_p = "1"; // skip unlink when no new photo
        }
                ...
                $insert_transaction = mysqli_query($con,"insert into `transaction`(`userid`, `des`, `debit`, `type`, `date`, `c_date`, `op_bal`, `cl_bal`) values('$user_details[id]', '$des', '$book_course_details[fee]', '1', '$t_date', '$c_date', '$op_bal', '$cl_bal')");
                 if($insert_transaction){
                    $update_book_course=mysqli_query($con,"update course_book set session_id='$selected_session_id', status='RUN' where id='$book_course_details[id]'") ;
                    if($update_book_course){
                        
                        if($fee_collect_type == "YES") {
                                $update_user_data = mysqli_query($con,"update user set fee_collect_type='YES', monthly_fee='$monthly_fee', daily_late_fee='$daily_late_fee', late_fee_count_after='$late_fee_count_after', next_fee_date='$t_date' where id='$user_details[id]'");
                                ...
                                if($update_user_data){
                                    if($discount_in_rs > 0){
                                        ...
                                        if($update_wallet){
                                            $des = "Monthly Payment Discount Offer: ".$discount_in_per."% of ".$book_course_details['fee'];
                                            ...
                                            if(!$insert_transaction){
                                                echo '<script>alert("Transaction failed while applying monthly discount. Please try again.");window.location.assign("enroll_new");</script>';
                                                exit();
                                            }
                                        }else{
                                            echo '<script>alert("Failed to credit wallet with discount amount.");window.location.assign("enroll_new");</script>';
                                            exit();
                                        }
                                    }
                                    
                                    echo '<script>alert("Enrollment completed successfully with monthly fee plan.");window.location.assign("enroll_new");</script>';
                                } else {
                                    echo '<script>alert("Unable to save monthly fee plan details. Please contact support.");window.location.assign("enroll_new");</script>';
                                }
                            }
                         ...
                         if($fee_collect_type=="OTP"){
                           ...
                           $insert_transaction = mysqli_query($con,"insert into `transaction`(`userid`, `des`, `credit`, `type`, `date`, `c_date`, `op_bal`, `cl_bal`) values('$user_details[id]', '$des', '$course_final_fee', '2', '$t_date', '$c_date', '$op_bal', '$cl_bal')");
                                 $insert_fee_collect_details=mysqli_query($con,"insert into `fee_collect_details`(`userid`, `session_id`, `by_rcv`, `amt`, `date`) values('$user_details[id]', '$selected_session_id', '$_SESSION[userid]', '$course_final_fee', '$t_date')");
                                  if($insert_transaction){
                       ...
                       if($fee_collect_type == "NO"){ 
                        $update_user_data = mysqli_query($con,"update user set fee_collect_type='NO' where id='$user_details[id]'");
                        ...
                        if($update_user_data){
                            if($discount_in_rs > 0){
                                ...
                                $update_wallet = mysqli_query($con,"update wallet set main_b='$cl_bal' where userid='$user_details[id]'");
                                if($update_wallet){
                                    $des = "Part Payment Discount: ".$discount_in_per."% of ₹".$book_course_details['fee'];
                                    $insert_transaction = mysqli_query($con,"insert into transaction(userid, des, credit, type, date, c_date, op_bal, cl_bal) values('$user_details[id]', '$des', '$discount_in_rs', '2', '$t_date', '$c_date', '$op_bal', '$cl_bal')");
                                    
                                    if(!$insert_transaction){
                                        echo '<script>alert("Transaction failed while applying part payment discount."); window.location.assign("enroll_new");</script>';
                                        exit();
                                    }
                                } else {
                                    echo '<script>alert("Unable to credit discount to user wallet."); window.location.assign("enroll_new");</script>';
                                    exit();
                                }
                            }
                    
                            echo '<script>alert("Enrollment completed successfully with part payment option."); window.location.assign("enroll_new");</script>';
                        } else {
                            echo '<script>alert("Unable to update user for part payment setup."); window.location.assign("enroll_final");</script>';
                        }
                    }
              ...
              <!-- form start -->
                ...
                <div class="card-body">
                  <div class="row">
                    ...
                    <div class="col-md-2">
                      <label>Session:</label>
                      <select id="session_id" name="session_id" disabled class="form-control">
                        <option value="">Please select</option>
                        <?php
                        $sql_course = mysqli_query($con, "SELECT * FROM session_details ORDER BY id");
                        while ($row = mysqli_fetch_array($sql_course)) {
                          $selected = ($row['id'] == $selected_session_id) ? 'selected' : '';
                      ...
                      <input type="text" readonly name="reg_no" onkeyup="update_user_data('reg_no',this.value,'<?php echo $user_details['id'] ;?>')" value="<?php echo $user_details['reg_no'] ;?>" class="form-control" placeholder="Enter Registraion Number">
                      ...
                      <select id="batch_id" name="batch_id" required class="form-control" onchange="update_course_batch(this.value)">
                        ...
                        <option value="">Please select</option>
                        <?php
                        $sql_course=mysqli_query($con,"select * from batch_details where branch_id='$current_branch_id'");
                        while($row=mysqli_fetch_array($sql_course)){
                        ?>
              ...
              <!-- form start -->
                ...
                <div class="card-body">
                  <div class="row">
                      ...
                      <input type="text" required name="mother_name" onkeyup="update_user_data('mother_name',this.value,'<?php echo $user_details['id'] ;?>')" value="<?php echo $user_details['mother_name'] ;?>" class="form-control">
              ...
              <!-- form start -->
                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                      ...
                      <input type="text"  name="mobile" value="<?php echo $user_details['mobile'] ;?>" onkeyup="update_user_data('mobile',this.value,'<?php echo $user_details['id'] ;?>')" class="form-control">
                  </div>
                   <div class="col-md-6">
                ...
                <select name="gender"  id="gender"  required  class="form-control" onchange="update_user_data('gender',this.value,'<?php echo $user_details['id'] ;?>')">
                  ...
                  <script>
                   ...
                   <div class="col-md-6">
                <label>State:</label>
                <select name="state_id"  id="state_id"  required  class="form-control" onchange="update_user_data('state_id',this.value,'<?php echo $user_details['id'] ;?>')">
                   <option value="">Select </option>
                    <?php 
                    $state_sql = mysqli_query($con, "select * from states order by id");
                  ...
                  <script>
                    $("#state_id").val('<?php echo $user_details['state_id'] ;?>');
                  
                </script>
                
                <div class="col-md-6">
                      <label>Pin Code: </label>
                      <input type="number"  name="pin" id="pin" onchange="update_user_data('pin',this.value,'<?php echo $user_details['id'] ;?>')" required value="<?php echo $user_details['pin'] ;?>" class="form-control">
                  </div>
