...
$check = mysqli_query($con, "select id from login_session where userid='$login_details[id]'");
if(mysqli_num_rows($check) == 0){
    $session_id = mysqli_fetch_array(mysqli_query($con, "select id from session_details where status='RUN'"))['id'];
    $insert = mysqli_query($con, "insert into login_session (`userid`, `session_id`, `status`, `updated_at`) values('$login_details[id]', '$session_id', '1', NOW())");
}
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        $sql_book_course=mysqli_query($con,"select * from course_book where status='RUN' and session_id='$c_session'");
       ...
       if($branch_access_details['enquiry_system']=="YES"){
         if($_SESSION['userid']==1){
             $total_enquery=mysqli_num_rows(mysqli_query($con,"select * from enquiry_details where  session_id='$c_session'"));
             $total_enquery_run=mysqli_num_rows(mysqli_query($con,"select * from enquiry_details where status='RUN' and session_id='$c_session'"));
            $total_enquery_convert=mysqli_num_rows(mysqli_query($con,"select * from enquiry_details where status='CLOSE' and session_id='$c_session'"));
            $total_enquery_cancel=mysqli_num_rows(mysqli_query($con,"select * from enquiry_details where status='CANCEL' and session_id='$c_session'"));
             ?>
             <div class="col-lg-3 col-6">
            <!-- small box -->
            <div class="small-box bg-warning">
