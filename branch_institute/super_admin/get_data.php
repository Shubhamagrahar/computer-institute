...
if (isset($_GET['create_coupen'])) {
    ...
    foreach ($course_ids as $course_id) {
        ...
        $query = "INSERT INTO coupen_code (coupen_code, discount, course_id, create_date, valid_date) 
                  VALUES ('$coupen_code', '$discount', '$course_id', NOW(), '$valid_date')";
...
if (isset($_GET['validate_certificate_no'])) {
    $certNo = VerifyData($_GET['certificate_no']);
...
if(isset($_GET['update_contact'])){
    ...
    if($field != ""){
         $update=mysqli_query($con,"update website_data set $field='$data' where id='1'");
...
if(isset($_POST['create_course_type_ajax'])) {

    $course_type_name = trim($_POST['course_type_name'] ?? '');
    $photo = $_FILES["upload_file"]["name"] ?? '';
    $photo2 = $_FILES["upload_file"]["tmp_name"] ?? '';

    if($course_type_name != "" && $photo != "") {
        $check_name = mysqli_num_rows(mysqli_query($con, "SELECT * FROM course_type WHERE name='".mysqli_real_escape_string($con, $course_type_name)."'"));
        ...
        if($check_name == 0) {
            ...
            if(move_uploaded_file($photo2, "../".$photo_dr)) {
                $insert = mysqli_query($con, "INSERT INTO `course_type`(`name`, `img`) VALUES ('".mysqli_real_escape_string($con, $course_type_name)."', '".mysqli_real_escape_string($con, $photo_dr)."')");
                
                if($insert) {
                    echo "success:Course type created successfully.";
                } else {
                    echo "error:Server error 101.";
                }
            } else {
                echo "error:Image uploading failed.";
            }
        } else {
            echo "error:This course type name already exists.";
        }
...
if (isset($_POST['create_subject'])) {
    ...
    if ($subject_name !== "") {
        ...
        if ($check == 0) {
            ...
            echo "This subject name already exists.";
        }
    } else {
...
if (isset($_GET['update_subject'])) {
    $id = VerifyData($_GET['subject_id']);
    $subject_name = VerifyData($_GET['subject_name']);

    if ($id != "" && $subject_name != "") {
        $check = mysqli_query($con, "SELECT * FROM subject_details WHERE subject_name='$subject_name' AND id != '$id'");
        if (mysqli_num_rows($check) == 0) {
...
if (isset($_GET['change_status_course_type'])) {
    $id = (int) $_GET['id'];
    $field = $_GET['field'];
    $status = $_GET['status'];

    if (!in_array($status, ['SHOW', 'HIDE']) || $field != 'status') {
        
        echo 'Invalid request';
        exit;
    }

    $update = mysqli_query($con, "UPDATE course_type SET `$field` = '$status' WHERE id = '$id'");
...
if (isset($_GET['change_status_course'])) {
    $id = (int) $_GET['id'];
    $field = $_GET['field'];
    $status = $_GET['status'];

    $allowed_fields = ['status', 'first_page_status'];
    $allowed_status = ['OPEN', 'CLOSE'];

    if (!in_array($field, $allowed_fields) || !in_array($status, $allowed_status)) {
        echo "Invalid request";
        exit;
    }

    if ($field === 'first_page_status') {
        if ($status === 'CLOSE') {
            $go_next = 1;
        } else {
            $count_first_page_open_course = mysqli_num_rows(
                mysqli_query($con, "SELECT * FROM course_details WHERE status='OPEN' AND first_page_status='OPEN'")
            );

            if ($count_first_page_open_course >= 3) {
                echo "You have already shown 3 courses on the first page. Please hide one course and try again.";
                exit;
            } else {
                $go_next = 1;
            }
        ...
        if ($go_next === 1) {
            $update = mysqli_query($con, "UPDATE course_details SET `$field` = '$status' WHERE id = '$id'");
            if ($update) {
                echo "success";
            } else {
                echo "Database update failed";
...
if (isset($_GET['change_lms_content'])) {
 

    $id = VerifyData($_GET['id']);
    $field = VerifyData($_GET['field']);
    $status = VerifyData($_GET['status']); 
    $table = VerifyData($_GET['table']);



    if (!empty($id) && !empty($field) && ($status === 'SHOW' || $status === 'HIDE')) {
        $update = mysqli_query($con, "UPDATE $table SET $field='$status' WHERE id='$id'");
...
if (isset($_GET['change_director_status'])) {
    $id = (int) $_GET['id'];
    $field = $_GET['field'];
    $status = $_GET['status'];
    ...
    $update = mysqli_query($con, "UPDATE web_director_message SET `$field` = '$status' WHERE id = '$id'");
