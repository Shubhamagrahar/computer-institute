              ...
              $sql=mysqli_query($con,"select * from test_series_questions_type_details where test_series_type_id='$series_id' order by rand()");
               while($row_data=mysqli_fetch_array($sql)){
                   ...
                   if(!$check>0){
                       ...
                       if($question_details){
                           $ans_final=$question_details[$question_details['ans_final']];
                           $question_details_id=utf8_encode($question_details['id']);
                           $question_details_question=utf8_encode($question_details['test_question']);
                           $question_ans_a=utf8_encode($question_details['ans_a']);
                           $question_ans_b=utf8_encode($question_details['ans_b']);
                           $question_ans_c=utf8_encode($question_details['ans_c']);
                           $question_ans_d=utf8_encode($question_details['ans_d']);
                           $insert_question=mysqli_query($con,"insert into `test_series_at_question`(`test_series_id`, `question_id`, `question`, `ans1`, `ans2`, `ans3`, `ans4`, `correct_ans`) values('$test_series_id', '$question_details_id', '$question_details_question', '$question_ans_a', '$question_ans_b', '$question_ans_c', '$question_ans_d', '$ans_final')");
               ...
               if($update){
                 ...
                 echo '<script>alert("Started Test.");window.location.assign("test_series_quies");</script>';    
               }else{
                                    ...
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                <form method="post" name="start_form<?php echo $row['id'] ; ?>">
                                                     <div class="row">
                                                     ...
                                                     <div class="col-md-4 form-group">
                                                       ...
                                                       <input type="submit" name="start" id="start<?php echo $row['id']; ?>" class="btn btn-success" onclick="go_process('<?php echo $row['id']; ?>')" value="Start" required>  
