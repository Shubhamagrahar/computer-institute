...
if(isset($_POST['Update'])){
   ...
   if(!$franchise_name=="" and !$owner_name=="" and !$mob=="" and !$email=="" and !$state_id=="" and !$pin=="" and !$full_add=="" and !$w_mob=="" and !$student_course_fess=="" and !$student_certificate_fess==""){
      ...
      if(!$pay_per_course==""){
       $check_mobile=mysqli_num_rows(mysqli_query($con,"select * from user where mobile='$mob'"));
            if(!$check_mobile>0 or $mob==$update_user_details['mobile']){
               ...
               if($upload_photo){
                   $update=mysqli_query($con,"update user set name='$franchise_name', mobile='$mob', father_name='$owner_name', email='$email', w_mob='$w_mob', state_id='$state_id', pin='$pin', full_add='$full_add' where id='$update_user_details[id]'"); 
                    if($update){
