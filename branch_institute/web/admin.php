...
if(isset($_POST['login'])){
    ...
    if(!$mobile=="" and !$pass==""){
     $sql=mysqli_query($con,"select * from website_data where user='$mobile' and pass='$pass'");
     if(mysqli_num_rows($sql)==1){
         $result=mysqli_fetch_array($sql);
         
         if($result['user']=="$mobile" and $result['pass']=="$pass"){
              
                 $_SESSION['id']=session_id();
                 $_SESSION['userid']=$result['id'];
                 $_SESSION['login_type']='super_cjgfdsaqertyuimbvcxsdsrd45tr8778hj5t4ry63b6f';
                 echo '<script>window.location.assign("super_admin/");</script>';
             
          
          
         }else{
          echo '<script>alert("Mobile number or password is not correct."); window.location.assign("admin");</script>';   
         }
         
     }else{
       echo '<script>alert("Please fill correct mobile number and password."); window.location.assign("admin");</script>';  
     }
        
    }else{
        echo '<script>alert("Please fill mobile number and password."); window.location.assign("admin");</script>';
    }
...
//     if(!$user==""){
//         $sql_chk=mysqli_query($con,"select * from website_data where user='$user'");
...
//                 $update=mysqli_query($con,"update website_data set pass='$rand_pass' where id='$result[id]'");
...
//                 $send=  send_mail2("$email","Forget Password Details ","$text");
//                 if($send){
//                      echo '<script>alert("Your password has been sent to your registered email id."); window.location.assign("admin");</script>';
                   
//                 }else{
//                     echo '<script>alert("Server error, Try Again."); window.location.assign("admin");</script>';
                  
//                 }
                
//             }else{
//                 echo '<script>alert("Somthing is wrong."); window.location.assign("admin");</script>';
             
//             }
            
//         }else{
//              echo '<script>alert("Mobile or email is not registered."); window.location.assign("admin");</script>';
            
//         }
        
//     }else{
//         echo '<script>alert("Please enter email or mobile no."); window.location.assign("admin");</script>';
